...
const Navbar = () => {
  ...
  const navItems = [
                    {name:"HOME",Rroute:"/"}, 
                    {name:"About",Rroute:"/About"},
                    {name:"Features",Rroute:"/eatures"},
                    {name:"About us",Rroute:"/AboutUs"}
                  ];
      ...
      <div className="flex justify-between items-center h-24 bg-black shadow-lg" >
        ...
        <div className="hidden md:flex">
          <motion.div whileHover={{ scale: 1.1 }}>
            <Button variant="outline" className="h-24 px-8 text-lg font-semibold" onClick={()=>{
              if(!isLoggedIn){
                route.push('/login')
              }else{
                route.push('/LogOut')
        ...
        <div className="md:hidden bg-black/90 backdrop-blur-sm absolute top-24 left-0 w-full px-4 py-4 space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <motion.div whileHover={{ scale: 1.1 }}>
              <Button
                variant="outline"
                className="w-full px-8 py-2 text-lg font-semibold"
            ...
            <motion.div>
              <Button
                variant="destructive"
                className="w-full bg-[#FF90E8] hover:bg-green-600 px-6 py-2 text-lg font-bold"
                onClick={() => setIsMenuOpen(false)}
              >
                Selling Details
              </Button>
