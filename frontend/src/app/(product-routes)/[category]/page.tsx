...
const Page = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/v1/api/getProductCategory`,
           { category },{
            headers: {
  ...
  return (
   <div className="min-h-screen p-8 max-w-4xl mx-auto">
      ...
      {error && (
        <div className="p-4 mb-4 text-sm text-pink-500 border border-pink-500 rounded-lg">
          Error: {error}
        </div>
      )}
          ...
          >
            ...
            <div className="w-2/3 flex flex-col justify-between">
              <div>
                ...
                <h2 className="text-2xl font-bold  mb-2 bg-green-500 px-4 text-zinc-900">{product.name}</h2>
                <span className="px-3 py-1 bg-yellow-500 text-zinc-900  text-sm">
                    {product.type}
                  </span>
                </div>
                <p className="text-gray-400 bg-slate-400/10 text-sm mb-4 line-clamp-2">{product.description}</p>
              </div>
